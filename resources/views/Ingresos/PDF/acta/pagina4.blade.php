  ...
  <div class="col-xs-12 text-justify">
    <p class="col-xs-12">
      @if ($ingreso->f_paciente != $ingreso->f_responsable)
        Yo, <b class="subrayar">{{$ingreso->responsable->nombre.' '.$ingreso->responsable->apellido}}</b> de <b class="subrayar">{{$ingreso->responsable->fechaNacimiento->age}}</b> años, con Documento Único de Identidad número:&nbsp; <b class="subrayar">
					@if ($ingreso->responsable->dui != null)                    {{$ingreso->responsable->dui}}
					@else
    ...
    <p class="col-xs-12">
      3 – Que estoy consciente del estado de salud de mi {{($ingreso->f_paciente != $ingreso->f_responsable)?"pariente":"persona"}} y la posibilidad de riesgo que conlleva toda ejecución de procedimientos médicos, por lo que {{($ingreso->medico->sexo)?"el doctor":"la doctora"}} me ha explicado de forma amplia y razonable sobre los mismos.
    </p>
    <p class="col-xs-12">
    ...
    <p class="col-xs-12">
      Por lo tanto, me manifiesto conforme y autorizo {{($ingreso->medico->sexo)?"al doctor":"a la doctora"}} que proceda a efectuar lo recomendado hacia la patología clínica de mi {{($ingreso->f_paciente != $ingreso->f_responsable)?"pariente":"persona"}} en el Hospital “Divino Niño”, institución que se encuentra debidamente inscripta en las instancias correspondientes para su operatividad.
    </p>
    <p class="col-xs-12">
